...
install_plugins() {
    ...
    while IFS= read -r plugin_line || [[ -n "$plugin_line" ]]; do
        ...
        local plugin load_type
        read -r plugin load_type <<< "$plugin_line"
        ...
        if [ ! -d "$plugin_cache_path" ]; then
            ...
            # Ensure the parent directory for the cache path exists
            mkdir -p "$(dirname "$plugin_cache_path")"
...
# It loads all installed plugins in the order specified in plugins.txt.
# Do not edit manually.

EOF

    if [ ! -f "$plugin_file" ]; then
        log_info "-> No plugins.txt found, no plugins to load."
        return
    fi
...
if test -d "$install_path"
    for fish_file in "\$install_path"/*.fish
        if test -f "\$fish_file"
            source "\$fish_file"
        end
    end
